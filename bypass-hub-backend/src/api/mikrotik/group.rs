...
async fn list(State(state): State<AppState>) -> impl IntoResponse {
    match db::mikrotik::group::find_all(&state.pool).await {
...
async fn create(
    ...
    match db::mikrotik::group::create(&state.pool, &body.name, body.description.as_deref()).await {
...
async fn remove(State(state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    match db::mikrotik::group::delete(&state.pool, id).await {
...
async fn get_routes(State(state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    match db::mikrotik::group::find_routes(&state.pool, id).await {
...
async fn add_route(
    ...
    match db::mikrotik::group::add_route(&state.pool, id, body.route_id).await {
...
async fn remove_route(
    ...
    match db::mikrotik::group::remove_route(&state.pool, id, route_id).await {
        Ok(true) => StatusCode::NO_CONTENT.into_response(),
        Ok(false) => StatusCode::NOT_FOUND.into_response(),
        Err(e) => {
            eprintln!("DB error: {e}");
