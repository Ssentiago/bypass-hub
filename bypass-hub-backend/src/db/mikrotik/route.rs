...
pub async fn find_all(pool: &SqlitePool) -> sqlx::Result<Vec<Route>> {
    sqlx::query_as!(
        Route,
        "SELECT id, value, type as \"type\" FROM mikrotik_routes"
    )
    .fetch_all(pool)
    .await
}
...
pub async fn create(pool: &SqlitePool, value: &str, route_type: &str) -> sqlx::Result<i64> {
    let id = sqlx::query!(
        "INSERT INTO mikrotik_routes (value, type) VALUES (?, ?)",
        value,
        route_type
    )
    .execute(pool)
...
pub async fn create_bulk(pool: &SqlitePool, routes: &[(&str, &str)]) -> sqlx::Result<u64> {
    ...
    for (value, route_type) in routes {
        let result = sqlx::query!(
            "INSERT OR IGNORE INTO mikrotik_routes (value, type) VALUES (?, ?)",
            value,
            route_type
        )
        .execute(pool)
...
pub async fn find_grouped(pool: &SqlitePool) -> sqlx::Result<Vec<GroupedRoutes>> {
    let rows = sqlx::query!(
        ...
        LEFT JOIN mikrotik_routes_groups rg ON rg.route_id = r.id
        LEFT JOIN mikrotik_groups g ON g.id = rg.group_id
...
pub async fn delete(pool: &SqlitePool, id: i64) -> sqlx::Result<bool> {
    let affected = sqlx::query!("DELETE FROM mikrotik_routes WHERE id = ?", id)
        .execute(pool)
        .await?
        .rows_affected();
