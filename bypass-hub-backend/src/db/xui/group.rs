...
pub async fn find_all(pool: &SqlitePool) -> sqlx::Result<Vec<Group>> {
    sqlx::query_as!(Group, "SELECT id, name, description FROM xui_groups")
        .fetch_all(pool)
        .await
}
...
pub async fn create(pool: &SqlitePool, name: &str, description: Option<&str>) -> sqlx::Result<i64> {
    let id = sqlx::query!(
        "INSERT INTO xui_groups (name, description) VALUES (?, ?)",
        name,
        description
    )
    .execute(pool)
...
pub async fn delete(pool: &SqlitePool, id: i64) -> sqlx::Result<bool> {
    let affected = sqlx::query!("DELETE FROM xui_groups WHERE id = ?", id)
        .execute(pool)
        .await?
        .rows_affected();
...
pub async fn find_routes(pool: &SqlitePool, group_id: i64) -> sqlx::Result<Vec<Route>> {
    sqlx::query_as!(
        ...
        "SELECT r.id, r.value, r.type as \"type\"
         FROM xui_routes r
         JOIN xui_routes_groups rg ON rg.route_id = r.id
         WHERE rg.group_id = ?",
        group_id
    )
    .fetch_all(pool)
...
pub async fn add_route(pool: &SqlitePool, group_id: i64, route_id: i64) -> sqlx::Result<()> {
    sqlx::query!(
        "INSERT OR IGNORE INTO xui_routes_groups (route_id, group_id) VALUES (?, ?)",
        route_id,
        group_id
    )
    .execute(pool)
...
pub async fn remove_route(pool: &SqlitePool, group_id: i64, route_id: i64) -> sqlx::Result<bool> {
    let affected = sqlx::query!(
        "DELETE FROM xui_routes_groups WHERE group_id = ? AND route_id = ?",
        group_id,
        route_id
    )
    .execute(pool)
